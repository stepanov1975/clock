...
// multiple RGBmatrixPanel_my instances.  The solution at present is to
...
static RGBmatrixPanel_my *activePanel = NULL;

// Code common to both the 16x32 and 32x32 constructors:
void RGBmatrixPanel_my::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
  uint8_t sclk, uint8_t latch, uint8_t oe, boolean dbuf, uint8_t width) {

 _width=WIDTH=width;
 _height=HEIGHT=rows*2;
...
RGBmatrixPanel_my::RGBmatrixPanel_my(
...
void RGBmatrixPanel_my::begin(void) {
...
// Original RGBmatrixPanel_my library used 3/3/3 color.  Later version used
// 4/4/4.


void RGBmatrixPanel_my::drawPixel(uint8_t x, uint8_t y, uint8_t r, uint8_t g,uint8_t b) {
  uint8_t bit, limit, *ptr;

  if((x < 0) || (x >= _width) || (y < 0) || (y >= _height)) return;
  if (upsidedown==1) y=_height-y-1;
  if (flip==1) x=_width-x-1;
...
void RGBmatrixPanel_my::cleanScreen(){
    // For black or white, all bits in frame buffer will be identically
    // set or unset (regardless of weird bit packing), so it's OK to just
    // quickly memset the whole thing:
    memset(matrixbuff[backindex], 0x0000, WIDTH * nRows * 3);
}


// Return address of back buffer -- can then load/store data directly
uint8_t *RGBmatrixPanel_my::backBuffer() {
...
void RGBmatrixPanel_my::swapBuffers(boolean copy) {
...
void RGBmatrixPanel_my::dumpMatrix(void) {
...
void RGBmatrixPanel_my::updateDisplay(void) {
